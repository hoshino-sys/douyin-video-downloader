...
class APIServer(TikTok):
    ...
    def setup_routes(self):
        ...
        async def index():
            ...
            template_path = Path("src/web_ui/templates/index.html")
            if template_path.exists():
                return HTMLResponse(content=template_path.read_text(encoding="utf-8"))
            return RedirectResponse(url=REPOSITORY)

        @self.server.post("/api/download")
        async def api_download(req: DownloadRequest):
            try:
                ...
                if req.type == "detail":
                    ...
                    data_list = [req.data]
                    # 调用下载器
                    # 注意：self.downloader.run 是异步的
                    # 我们这里希望在后台运行，不阻塞 API
                    # 但 downloader.run 本身是 async 的，await 它会等待下载完成
                    # 如果想后台运行，可以使用 asyncio.create_task
                    import asyncio
                    asyncio.create_task(self.downloader.run(
                        data_list,
                        "detail",
                        tiktok=is_tiktok
                    ))
                    ...
                    if not req.id:
                        return {"status": "failed", "message": "No ID provided"}
                    
                    import asyncio
                    asyncio.create_task(self.deal_account_detail(
                        index=0,
                        sec_user_id=req.id,
                        tab=req.tab,
                    ...
                    # 合集下载
                     ...
                     import asyncio
                     # 需要调用 mix 相关的处理逻辑
                     # 查看 handle_mix: is_mix, id_ = self.generate_mix_params(...)
                     # deal_mix_detail(...)
                     
                     is_mix, id_ = self.generate_mix_params(req.id, "")
                     if not isinstance(is_mix, bool):
                         return {"status": "failed", "message": "Invalid Mix ID"}
                     
                     asyncio.create_task(self.deal_mix_detail(
                        is_mix,
                        id_,
                        api=False, # 触发下载
                        tiktok=is_tiktok
                ...
                return {"status": "failed", "message": "Unknown type"}
